...
public class Functions {
	...
	public static void displayRates(CommandSender sender, VaultSupport vault, configHandler config) { // Added argument: VaultSupport, configHandler (turt2live)
		if (vault.hasRegister()) { // Added by turt2live (rate table for Register is EdTheLoon)
			...
			if (!config.convertDiamond && !config.convertGold && !config.convertIron) sender.sendMessage(ChatColor.RED + "This server doesn't allow anything to be converted");
			else {
				sender.sendMessage("--- Conversion Rates ---");

				if (config.convertIron) sender.sendMessage("1 " + ChatColor.GRAY + "Iron" + ChatColor.WHITE + " = "
						+ ChatColor.GREEN + method.format(config.ironRate));

				if (config.convertGold) sender.sendMessage("1 " + ChatColor.YELLOW + "Gold" + ChatColor.WHITE + " = "
						+ ChatColor.GREEN + method.format(config.goldRate));

				if (config.convertDiamond) sender.sendMessage("1 " + ChatColor.AQUA + "Diamond" + ChatColor.WHITE + " = "
						+ ChatColor.GREEN + method.format(config.diamondRate));
			}
		} else if (!config.convertDiamond && !config.convertGold && !config.convertIron) sender.sendMessage(ChatColor.RED + "This server doesn't allow anything to be converted");
		else {
			sender.sendMessage("--- Conversion Rates ---");
